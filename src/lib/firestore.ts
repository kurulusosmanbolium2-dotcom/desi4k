...
export async function getAllVideos(): Promise<Video[]> {
  const snap = await getDocs(query(collection(db, "video"), orderBy("views", "desc")));
  return snap.docs.map((d) => ({ id: d.id, ...d.data() } as Video));
}
export async function getVideo(id: string): Promise<Video | null> {
  const snap = await getDoc(doc(db, "video", id));
  return snap.exists() ? { id: snap.id, ...snap.data() } as Video : null;
}
export async function incrementViews(id: string) {
  await updateDoc(doc(db, "video", id), { views: increment(1) });
                                                           }
